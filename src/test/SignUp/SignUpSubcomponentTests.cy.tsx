...
describe('<SignUpTabItem />', () => {
  it('renders with text', () => {
    ...
    cy.mount(<SignUpTabItem text={text} waiting={false} complete={false} />)
    cy.contains(text)
  })

  it('shows CircularProgress when waiting is true', () => {
    cy.mount(<SignUpTabItem text="Waiting" waiting={true} complete={false} />)
    cy.get('.MuiCircularProgress-root').should('exist')
  })

  it('shows CheckCircle when complete is true', () => {
    cy.mount(<SignUpTabItem text="Complete" waiting={false} complete={true} />)
    cy.get('.MuiSvgIcon-root').should('exist')
  })

  it('does not show CircularProgress and CheckCircle when both are false', () => {
    cy.mount(<SignUpTabItem text="None" waiting={false} complete={false} />)
