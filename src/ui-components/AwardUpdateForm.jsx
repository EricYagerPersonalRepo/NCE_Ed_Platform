...
import { getAward } from "../graphql/queries";
import { updateAward } from "../graphql/mutations";
const client = generateClient();
export default function AwardUpdateForm(props) {
  const {
    id: idProp,
    award: awardModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = awardRecord
      ? { ...initialValues, ...awardRecord }
      : initialValues;
    setAwardDate(cleanValues.awardDate);
    setAwardSource(cleanValues.awardSource);
  ...
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getAward.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAward
        : awardModelProp;
    ...
    queryData();
  }, [idProp, awardModelProp]);
  React.useEffect(resetStateValues, [awardRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      {...getOverrideProps(overrides, "AwardUpdateForm")}
      {...rest}
    >
      <TextField
        label="Award date"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || awardModelProp)}
